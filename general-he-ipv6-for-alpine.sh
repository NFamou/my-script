...
# 使用更通用的 id -u 判断 root，兼容 Alpine 的默认 ash
if [ "$(id -u)" -ne 0 ]; then
  echo "❌ 请使用 root 用户运行"
  exit 1
fi
...
if [ "$OS_FAMILY" = "alpine" ]; then
  apk update
  # Alpine 的包名略有不同
  apk add ip6tables iproute2 curl wget sudo net-tools

elif [ "$OS_FAMILY" = "debian" ]; then
  apt update
  apt install -y \
    curl wget sudo iptables iproute2 net-tools iptables-persistent

elif [ "$OS_FAMILY" = "rhel" ]; then
  dnf install -y \
    curl wget sudo iptables iptables-services iproute net-tools
fi

# =============================
# 启用 ip6tables 服务（仅 RHEL 需要预先启动）
# =============================

...
# v2bx-install-fixed.sh - V2bX 安装脚本（仅对 raw.githubusercontent.com 使用 gh-proxy.com 代理）

# Usage: sudo bash v2bx-install-fixed.sh [version]
...
# 仅代理 raw.githubusercontent.com 的前缀（若不想代理可置空）
...
apk update || true
apk add --no-cache wget curl unzip tar socat ca-certificates || true
update-ca-certificates || true
elif [[ "${release}" == "debian" || "${release}" == "ubuntu" ]]; then
apt-get update -y || true
DEBIAN_FRONTEND=noninteractive apt-get install -y wget curl unzip tar cron socat ca-certificates || true
update-ca-certificates || true
...
last_version="$(curl -s "[https://api.github.com/repos/wyx2685/V2bX/releases/latest](https://api.github.com/repos/wyx2685/V2bX/releases/latest)" | grep -m1 '"tag_name":' | sed -E 's/.*"([^"]+)".*/\1/' || true)"
if [[ -z "${last_version}" ]]; then
last_version="$(curl -sI -L [https://github.com/wyx2685/V2bX/releases/latest](https://github.com/wyx2685/V2bX/releases/latest) | grep -i location | tail -n1 | awk -F'/' '{print $NF}' | tr -d $'\r' || true)"
fi
if [[ -z "${last_version}" ]]; then
echo -e "${red}检测 V2bX 版本失败，请手动指定版本安装，例如：sudo bash v2bx-install-fixed.sh v0.4.0${plain}"
exit 1
fi
else
...
zip_url="[https://github.com/wyx2685/V2bX/releases/download/${last_version}/V2bX-linux-${arch}.zip](https://github.com/wyx2685/V2bX/releases/download/${last_version}/V2bX-linux-${arch}.zip)"
...
v2bx_sh_raw_url="[https://raw.githubusercontent.com/wyx2685/V2bX-script/master/V2bX.sh](https://raw.githubusercontent.com/wyx2685/V2bX-script/master/V2bX.sh)"
...
cd "${cur_dir}" || true
rm -f v2bx-install-fixed.sh || true

echo -e ""
echo -e "${green}V2bX ${last_version:-unknown} 安装完成（raw.githubusercontent.com 的脚本下载已使用代理处理）。${plain}"
...
install_base
install_V2bX "${1:-}" || { echo -e "${red}安装过程遇到错误，已退出。请检查上方输出并重试。${plain}"; exit 1; }
exit 0
